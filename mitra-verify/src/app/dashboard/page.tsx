...
import { 
  KpiCardSkeleton, AreaChartSkeleton, SummarySkeleton, 
  ApiPerfSkeleton, TableSkeleton, SystemStatusSkeleton 
} from '@/components/dashboard/Skeletons';
import React from 'react';

const DashboardAreaChart = dynamic(() => import('@/components/dashboard/Charts').then(mod => mod.DashboardAreaChart), { ssr: false, loading: () => <AreaChartSkeleton /> });
const SparklineChart = dynamic(() => import('@/components/dashboard/Charts').then(mod => mod.SparklineChart), { ssr: false });
const TinyLineChart = dynamic(() => import('@/components/dashboard/Charts').then(mod => mod.TinyLineChart), { ssr: false });
...
export default function DashboardPage() {
  ...
  const { data: telemetry, isLoading: loading, error: queryError, refetch } = useQuery({
    queryKey: ['dashboard', timeframe],
    queryFn: async () => {
      const res = await analyticsAPI.dashboard(timeframe);
    ...
    <ProtectedRoute>
      <div className="min-h-screen bg-[#070B17] font-sans text-slate-300 selection:bg-[#00D4FF]/30 pb-20">
        ...
        >
           ...
           {error && !telemetry ? (
             <div className="bg-[#0F172A] border border-white/10 rounded-[18px] p-8 w-full shadow-2xl text-center my-10">
               <ShieldAlert size={48} className="text-[#EF4444] mx-auto mb-4 opacity-80" />
               <h2 className="text-[20px] font-semibold text-white mb-2">Dashboard Unavailable</h2>
               <p className="text-[14px] text-slate-400 mb-6">{error}</p>
               <button 
                 onClick={() => refetch()}
                 className="bg-[#00D4FF]/10 text-[#00D4FF] border border-[#00D4FF]/20 px-6 py-2.5 rounded-full text-[14px] font-medium hover:bg-[#00D4FF]/20 transition-colors"
               >
                 Try Again
               </button>
             </div>
             ...
             <>
               ...
               <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 md:gap-5">
                 <ApiPerfSkeleton /><ApiPerfSkeleton /><ApiPerfSkeleton />
               </div>
             ...
             <>
               ...
               {/* ========================================================= */}
               <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3 md:gap-5">
                 <KpiCard title="Total Requests" value={executive_overview.total_requests.toLocaleString()} trend="▲ 8.2%" trendUp icon={Activity} sparklineData={sparklineData} dataKey="total" color={COLORS.accent} />
                 <KpiCard title="Passed" value={executive_overview.successful_requests.toLocaleString()} trend="▲ 12.4%" trendUp icon={CheckCircle2} sparklineData={sparklineData} dataKey="passed" color={COLORS.success} />
                 <KpiCard title="Failed" value={executive_overview.failed_requests.toLocaleString()} trend="▼ 2.1%" trendUp={false} icon={ShieldAlert} sparklineData={sparklineData} dataKey="failed" color={COLORS.failed} />
                 <KpiCard title="Spoof Attempts" value={executive_overview.spoof_attempts.toLocaleString()} trend="▲ 4.3%" trendUp icon={ShieldAlert} sparklineData={sparklineData} dataKey="spoof" color={COLORS.spoof} />
                 <KpiCard title="Face Lost" value={executive_overview.face_lost.toLocaleString()} trend="▼ 1.6%" trendUp={false} icon={EyeOff} sparklineData={sparklineData} dataKey="face_lost" color={COLORS.warning} />
                 <KpiCard title="Average Response Time" value={`${executive_overview.avg_latency}ms`} trend="▼ 5.6%" trendUp={false} icon={Clock} sparklineData={sparklineData} dataKey="latency" color={COLORS.accent} />
               </div>
               ...
               <div className="grid grid-cols-1 xl:grid-cols-3 gap-3 md:gap-5">
                 ...
                 <motion.div variants={itemVariants} className="xl:col-span-2 bg-[#0F172A] border border-white/5 rounded-[18px] p-4 md:p-6 shadow-sm flex flex-col">
                   ...
                   <div className="h-[220px] sm:h-[250px] md:h-[280px] w-full mt-2">
                     <DashboardAreaChart timeline={timeline} />
                   </div>
                 </motion.div>
                        ...
                        >
                           ...
                           <div className="flex items-end justify-between gap-2">
                              ...
                              <div className="w-16 h-8">
                                <TinyLineChart data={sparklineData.slice(-10)} color={color} />
                              </div>
                           </div>
                        </motion.div>
                     )
                             ...
                             {filteredEvents.length === 0 ? (
                               <tr>
                                 ...
                                 filteredEvents.map(ev => {
                                   const isPassed = ev.status === 'VERIFIED' || ev.status === 'IDENTITY MATCHED' || ev.status === 'SUCCESS';
                                   const isFaceLost = ev.status === 'NO FACE DETECTED';
                                   const isSpoof = ev.status === 'SPOOF ATTEMPT' || ev.threat > 50;
                                 ...
                                 return (
                                   <tr key={ev.id} className="hover:bg-white/[0.02] transition-colors group cursor-pointer">
                                      ...
                                      <td className="px-5 py-3 text-[12px] text-slate-400">{ev.api === 'Basic' ? 'API 1' : ev.api === 'Advanced' ? 'API 2' : 'API 3'}</td>
                                      ...
                                      <td className="px-5 py-3 text-[12px] text-slate-400">{!isPassed ? ev.status : '—'}</td>
                                      <td className={`px-5 py-3 text-[12px] font-mono ${ev.latency > 600 ? 'text-[#F59E0B]' : 'text-[#10B981]'}`}>{ev.latency}ms</td>
...
const KpiCard = React.memo(function KpiCard({ title, value, trend, trendUp, icon: Icon, sparklineData, dataKey, color }: any) {
  return (
    <motion.div 
      variants={hoverGlow}
      initial="rest"
    ...
    >
       ...
       <div className="absolute bottom-6 left-0 right-0 h-10 opacity-[0.15] pointer-events-none px-2">
          <SparklineChart data={sparklineData} dataKey={dataKey} color={color} />
       </div>
    </motion.div>
  );
});
