    ...
    <div className="flex flex-col gap-2 overflow-hidden h-[300px] relative mt-4">
      ...
      <AnimatePresence initial={false}>
        {feed.map((item) => {
          ...
          if (item.type === 'SPOOF') { color = '#ffb800'; Icon = AlertTriangle; label = 'SPOOF ATTACK'; }
          else if (item.type === 'FAIL') { color = '#ff3366'; Icon = AlertTriangle; label = 'FAILED'; }
          else if (item.type === 'NO_FACE') { color = '#94a3b8'; Icon = Eye; label = 'NO FACE'; }
            ...
            >
              ...
              <div className="flex-1 min-w-0 flex flex-col">
                <div className="flex justify-between items-baseline">
                  ...
                  <span className="text-[10px] text-slate-500 font-mono">{format(item.timestamp, 'HH:mm:ss')}</span>
                </div>
